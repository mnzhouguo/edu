...
test('creates a weekly task, generates today, filters, and reorders',async({page})=>{
 ...
 await page.getByRole('button',{name:'今日看板',exact:true}).click();await expect(page.locator('.execution-task')).toHaveCount(3);
 await page.getByRole('button',{name:/数学\s*2/}).click();await expect(page.locator('.execution-task')).toHaveCount(2);await page.locator('.execution-task').filter({hasText:'数学练习任务二'}).getByTitle('上移').click();
 ...
 await page.reload();await page.getByRole('button',{name:'今日看板',exact:true}).click();await expect(page.locator('.execution-task').nth(1)).toContainText('数学练习任务二');
});
